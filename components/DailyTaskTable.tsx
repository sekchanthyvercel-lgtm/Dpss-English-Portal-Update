...
export const DailyTaskTable: React.FC<DailyTaskTableProps> = ({
  ...
  const NAME_START = isFrozen ? NUMBER_WIDTH : 0;
                    ...
                    <table className="w-full border-collapse table-fixed min-w-[1500px]">
                        <thead className="sticky top-0 z-40 bg-white/10 backdrop-blur-md">
                            <tr className="border-b border-white/5 uppercase text-[9px] font-black text-slate-800">
                                <th className={`w-10 py-5 text-center border-r border-white/5 sticky top-0 bg-white/[0.05] z-40 ${isFrozen ? 'left-0 z-50 bg-white shadow-[2px_0_0_0_rgba(0,0,0,0.05)]' : ''}`} style={{ width: NUMBER_WIDTH, left: isFrozen ? 0 : undefined }}>#</th>
                                <th className={`px-6 py-5 text-left border-r border-white/5 sticky top-0 z-50 transition-all group ${isFrozen ? 'bg-white shadow-[4px_0_10px_rgba(0,0,0,0.1)]' : 'bg-inherit'}`} style={{ width: studentNameWidth, left: isFrozen ? NAME_START : undefined }}>
                            ...
                            {filteredStudents.map((s, idx) => {
                                ...
                                return (
                                    <tr key={s.id} className={`h-12 transition-all hover:brightness-95 group ${rowBg} ${s.isHidden ? 'opacity-30' : ''}`}>
                                        <td className={`text-center font-bold text-[10px] text-indigo-900/60 border-r border-white/5 bg-inherit ${isFrozen ? 'sticky left-0 z-20 bg-white shadow-[2px_0_0_0_rgba(0,0,0,0.05)]' : ''}`} style={{ width: NUMBER_WIDTH, left: isFrozen ? 0 : undefined }}>
                                            <div className="min-h-[44px] flex items-center justify-center">
                                                {idx + 1}
                                            </div>
                                        </td>
                                        <td className={`px-6 border-r border-white/5 sticky z-30 transition-all ${isFrozen ? 'shadow-[4px_0_10px_rgba(0,0,0,0.05)] bg-white' : 'bg-inherit'}`} style={{ width: studentNameWidth, left: isFrozen ? NAME_START : undefined }}>
